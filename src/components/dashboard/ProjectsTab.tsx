...
const emptyForm = {
  ...
  latitude: "", longitude: "",
  showroom_count: "", shop_count: "", office_count: "", apartment_count: "",
};
...
const ProjectsTab: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    const payload: TablesInsert<"projects"> = {
      name: form.name, city: form.city, district: form.district || null,
      street: form.street || null, status: form.status,
    ...
    setOpen(false); setEditingId(null); setForm(emptyForm); fetchProjects();
  };

  const handleEdit = (p: Project) => {
    setEditingId(p.id);
    setForm({
      name: p.name, city: p.city, district: p.district ?? "", street: p.street ?? "",
      status: p.status, latitude: p.latitude?.toString() ?? "", longitude: p.longitude?.toString() ?? "",
      showroom_count: p.showroom_count?.toString() ?? "0", shop_count: p.shop_count?.toString() ?? "0",
      office_count: p.office_count?.toString() ?? "0", apartment_count: p.apartment_count?.toString() ?? "0",
    });
    setOpen(true);
  };
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-lg font-medium text-foreground">{isAr ? "المشاريع" : "Projects"}</h2>
        {canCreate && (
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-border py-12 text-center">
          <p className="text-sm font-light text-muted-foreground">{isAr ? "لا توجد مشاريع بعد" : "No projects yet"}</p>
        </div>
      ) : (
                ...
                <TableRow key={p.id}>
                  ...
                  <TableCell>
                    <span className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-light ${
                      p.status === "ready" ? "bg-green-50 text-green-700" : "bg-amber-50 text-amber-700"
